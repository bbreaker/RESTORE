...
getRegInfo <- function(siteIDs) {
  ...
  regList <- data.frame(siteNo = as.character(), beginUnregDate = as.character(), 
                        endUnregDate = as.character(), beginDatePk = as.character(), 
                        endDatePk = as.character(), beginDtDv = as.character(), 
                        endDateDv = as.character(), lat = as.numeric(),
                        long = as.numeric(), stringsAsFactors = F)
  ...
  for (i in seq(1, length(sitesX), 1)) {
    ...
    pkFileReg <- pkFile[grepl("5|6|C|1", pkFile$peak_cd),]
    
    # keep rows in the peak flow file that contain codes indicating 
    # regulation, channelization, or diversion
    pkFileUnreg <- pkFile[!grepl("5|6|C|1", pkFile$peak_cd),]
    ...
    # for sites that have some or no "altered" flows
      ...
      # add that information to the data frame
      regListNew <- data.frame(siteNo = sitesX[i], beginUnregDate = as.character(pkFileReg[1,3]), 
                               endUnregDate = as.character(pkFileReg[nrow(pkFileReg),3,]), 
  ...
  regList$beginDtDvNew <- if_else(is.na(regList$beginUnregDate), regList$beginDtDvNew, 
                                  firstDayNextMonth(regList$beginUnregDate))
  
  # create a new column of dates to start to think about a new end date
  # to use for this site if the peak flow file indicates 'altered' and 'unaltered' periods
  regList$endDateDvNew <- if_else(is.na(regList$beginUnregDate), regList$endDateDv, 
                                  lastDayPrevMonth(regList$endDateDv))
